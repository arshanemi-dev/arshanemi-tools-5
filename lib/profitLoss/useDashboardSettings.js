...
// Settings -> Save toggle), and the saved Brand list (preferences.brands —
// created ad hoc from the toolbar's Brand picker). All four live in the same
...
export function useDashboardSettings({ ads, dateRange, onLoadedPreferences, addToast }) {
  ...
  const latest = useRef({ ads, dateRange, layout, brands });
  useEffect(() => {
    latest.current = { ads, dateRange, layout, brands };
  }, [ads, dateRange, layout, brands]);
  ...
  const save = useCallback((partial = {}) => {
    ...
    const { ads: a, dateRange: dr, layout: l, brands: b } = latest.current;
    return putSettings({
