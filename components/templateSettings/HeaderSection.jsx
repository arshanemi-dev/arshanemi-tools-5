...
export default function HeaderSection({ draft, activeId: activeIdProp, onActiveId }) {
  ...
  const addHeader = () => {
    const item = makeHeader({ name: `Header ${headers.length + 1}`, type: 'number', source: 'manual' });
    addItem('headers', item);
    setActiveId(item.id);
  };

  const badge = (h) => (h.source === 'default' ? 'default' : h.source === 'extracted' ? 'sheet' : '');
  ...
  return (
    <div id="section-header" className="scroll-mt-24">
      ...
      />
      {!headers.length ? (
        <p className="rounded-xl border border-divider bg-background py-10 text-center text-sm text-subtle">No headers yet — add one.</p>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="rounded-xl border border-divider bg-background p-4">
            {!active ? (
              <p className="py-10 text-center text-sm text-subtle">Pick a header from the list on the left to edit it.</p>
            ) : (
